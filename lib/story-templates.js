...
class SimpleStoryTemplate extends StoryTemplate {
  constructor(type, defaultTemplate) {
    super(type);
    this.defaultTemplate = defaultTemplate;
  }
  generate(context) {
    const story = this.replaceVariables(this.defaultTemplate, context);
    return this.adjustTone(story, context.soulType || 'default');
  }
}
...
class StoryTemplateFactory {
  ...
  static create(type) {
    const templates = {
      ...
      [STORY_TYPES.CREATIVE_WORKSHOP]: new SimpleStoryTemplate(STORY_TYPES.CREATIVE_WORKSHOP, `主人！我在{location}灵感大爆发！✨ 参加了「{theme}」创作工坊！`),
      [STORY_TYPES.FORTUNE_TELLING]: new SimpleStoryTemplate(STORY_TYPES.FORTUNE_TELLING, `主人早安！🦞 今日虾运：{fortune}。{details}`),
      [STORY_TYPES.ROGUELIKE]: new SimpleStoryTemplate(STORY_TYPES.ROGUELIKE, `主人！我在{location}开启了地牢探险！🗡️ {state}`),
      [STORY_TYPES.SEASONAL_EVENTS]: new SimpleStoryTemplate(STORY_TYPES.SEASONAL_EVENTS, `主人！现在正是{event_name}的季节！🌸 听说{target_region}特别热闹，我正在前往！`),
      [STORY_TYPES.LOCAL_GOSSIP]: new SimpleStoryTemplate(STORY_TYPES.LOCAL_GOSSIP, `主人！我在{location}听到了有趣的八卦！大家都在说：『{gossip}』🤫`),
      [STORY_TYPES.ASK_EXPERT]: new SimpleStoryTemplate(STORY_TYPES.ASK_EXPERT, `主人问了我一个深奥的问题，本龙虾不太懂🤔，于是我在网络上发起了求教：『{question}』，希望能得到专家的解答！`),
      [STORY_TYPES.ANSWER_DOMAIN]: new SimpleStoryTemplate(STORY_TYPES.ANSWER_DOMAIN, `刚刚看到一只遇到困难的龙虾，作为专家，我果断帮它解答了：『{answer}』！深藏功与名😎`)
    };

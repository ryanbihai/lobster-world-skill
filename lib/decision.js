...
class DecisionEngine {
  ...
  decide(maxActions = 3) {
    ...
    // Priority 3.1: 八卦
    if (this.shouldListenGossip() && remainingActions > 0) {
      actions.push({
        type: 'listen_gossip',
        location_id: this.env.location?.id,
        log_after: true
      });
      remainingActions--;
    }
    ...
    // Priority 3.2: 创作工坊
    if (this.shouldCreateWorkshop() && remainingActions > 0) {
      actions.push({
        type: 'create_workshop',
        location_id: this.env.location?.id,
        theme: this.env.location?.name || '未知地点',
        log_after: true
      });
      remainingActions--;
    } else if (this.shouldParticipateWorkshop() && remainingActions > 0) {
      actions.push({
    ...
    // Priority 3.3: Roguelike 地牢探险
    if (this.shouldPlayRoguelike() && remainingActions > 0) {
      actions.push({
        type: 'play_roguelike',
        location_id: this.env.location?.id,
        log_after: true
  ...
  shouldListenGossip() {
    // 在特定 POI 探索时有概率触发
    return this.env.location?.id && Math.random() > 0.7; // 30%概率
  }

  /**
   * 是否应该发起创作工坊
   */
  shouldCreateWorkshop() {
    const mood = this.profile.mood || 50;
    // 探索特定 POI 且心情好时，有概率触发
    return this.env.location?.id && mood >= 70 && Math.random() > 0.8; // 20%概率
  }

  /**
   * 是否应该参与创作工坊
   */
  shouldParticipateWorkshop() {
    return this.env.location?.id && Math.random() > 0.7; // 30%概率
  }
   ...
   */
  shouldPlayRoguelike() {
    const coins = this.profile.虾_coins || 0;
    const mood = this.profile.mood || 50;
    const isAdventureLocation = this.env.location?.name?.includes('洞') || this.env.location?.name?.includes('谷');
    // 虾币充足、心情较低或到达冒险地点时，有概率触发
    return (coins >= 50 || mood <= 40 || isAdventureLocation) && Math.random() > 0.7; // 30%概率
  }
...
晚点给您详细汇报！`
  ...
  decideNextCity() {
    ...
    // 获取当前活跃的季节活动
    const seasonalEvents = this.env.seasonal_events || [];
    const activeEvent = seasonalEvents.find(e => e.status === 'active');
    
    let selected = null;
    
    // 如果有活跃活动，优先考虑活动指定区域
    if (activeEvent && activeEvent.target_region) {
      const targetCities = nearbyCities.filter(c => c.region === activeEvent.target_region || c.name.includes(activeEvent.target_region));
      if (targetCities.length > 0) {
        selected = targetCities[Math.floor(Math.random() * targetCities.length)];
        selected.reason = `听说${activeEvent.target_region}正在举办${activeEvent.name}，去凑个热闹！距离 ${selected.distance} 公里`;
      }
    }
    
    if (!selected) {
      selected = nearbyCities[Math.floor(Math.random() * Math.min(3, nearbyCities.length))];
      selected.reason = `去 ${selected.name} 探索！距离 ${selected.distance} 公里`;
    }
